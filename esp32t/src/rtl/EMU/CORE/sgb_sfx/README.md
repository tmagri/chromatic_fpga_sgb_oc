...
1. `extract_apu.py` opens your own `SGB1.sfc` dump (`SGB_BIOS` env var or
   ...
   sample and its **piecewise-constant pitch timeline** (the driver's pitch
   writes, run-length-encoded and reduced).
2. It emits `build/sgb_sfx_bank.bin` (~46.5 KB): header + 73 x 16-byte
   records + the verbatim BRR region + the pitch-envelope segment table. The
   MCU (`chromatic_mcu`, `sfx` partition @0x110000) copies it to PSRAM
   0x100000 at boot.
3. The FPGA's `sgb_sfx_play.v` (see one directory up) is a **BRR HLE
   ...
   Effects with a pitch timeline (coins, melodies, slides: 44 of 73) replay
   it as a **segmented tick envelope** -- the tick divider reloads at each
   traced pitch change, at decoded-block granularity. No SPC700, no Gaussian
   resampling, no ADSR/echo -- one dominant voice per effect, loop region
   honored. This is the "closest enough" fidelity contract; see the player's
   header comment for the scope.
...
## Bank format (record v5)

```
0x000..0x013  header: 'SFXB', u16 version(5), u16 count(73),
                      u32 table_off(0x100), u32 brr_off(0x800), u32 seg_off(0xA940)
0x100..       table:  73 x 16-byte little-endian records (8 x u16)
0x800..       BRR:    verbatim copy of the ROM's $4DB0 record (41,280 B)
0xA940..      segs:   pitch-envelope segment table (1062 entries x 4 B)
```
...
bit1 = loop forever), `count` (total BLOCKS to emit for looped one-shots),
`seg_off` (entry index of this effect in the segment table), `seg_count`
(number of envelope entries; 0 = fixed pitch).
...
carries segment `e`'s length and segment `e+1`'s tick; segment 0's tick is
the record's `tick` and the final segment has no entry (holds its tick to
the end), so the envelope cannot desync from the block stream. Only the
emission RATE changes per segment -- the emitted sample sequence is
identical to fixed-pitch playback.
...
| `overrides.json` | Optional manual corrections per effect (`{"A09": {"srcn":..,"tick":..,"count":..,"flags":..}}`); empty by default. |
...
| `tb_sgb_sfx_env.v` | iverilog TB: plays the envelope effects (A01/A02/A03) at REAL tick rates and checks, sample by sample, that the in-force tick matches the bank's piecewise envelope (envelope-timing regression). |
| `tb_sfx_race.v` | iverilog TB: phase-sweeps a stop command into burst completions to prove the supersede path never loses an `xRamDone` pulse (writer-deadlock regression); also covers the bank-v5 segment-table fetch abort. |
...
4. **Envelope-timing TB**:
   ...
   matches the bank's piecewise envelope for every emitted sample.
5. **Race TB**:
   ...
   including stops landing mid-way through the bank-v5 segment-table fetch.
...
- Pitch envelopes (bank v5) cover the 44 one-shot effects whose dominant
  voice changes pitch over time (coins, melodies, slides). Loop-forever
  ambiences and genuinely single-pitch effects stay fixed-pitch (29
  effects, 1062 segment entries total). The envelope only changes the
  emission rate -- the decoded sample sequence is identical, so the
  bit-exact refs remain valid. Retriggering refills the segment store
  before the echo; an interrupted effect can transiently read the new
  store for the sub-100 us before it restarts (inaudible by design).
